    ...
    <div className="fixed top-0 inset-x-0 z-40 w-full">
      <div className="flex justify-between items-center">
        <div className="flex flex-row items-center shrink px-3 md:px-5 lg:px-8 py-5">
          ...
          <div className="text-gray-700 text-xl font-bold">Zero</div>
        </div>
          ...
          <AnimatePresence>
            {cart?.item_quantity > 0 && (
              <motion.button 
                ...
                exit={{ x: 50 }}
                className="block pl-3 pr-5 py-2 w-[50px] text-gray-700 font-semibold rounded-l-full bg-yellow-500  hover:bg-red-500 shadow-md focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-opacity-50">
                {cart?.item_quantity || 0}
              </motion.button>
            )}
          </AnimatePresence>
